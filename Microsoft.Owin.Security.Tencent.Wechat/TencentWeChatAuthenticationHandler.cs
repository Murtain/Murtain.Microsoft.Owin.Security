...
{
    public class TencentWeChatAuthenticationHandler : AuthenticationHandler<TencentWeChatAuthenticationOptions>
    {
        ...
        public TencentWeChatAuthenticationHandler(ILogger logger,HttpClient httpClient)
        {
            this.logger = logger;
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                var tokenResponse = await this.httpClient.GetAsync(tokenEndpoint, Request.CallCancelled);
                ...
                var userResponse = await this.httpClient.GetAsync(userInformationEndpoint, Request.CallCancelled);
                userResponse.EnsureSuccessStatusCode();

                var userString = await userResponse.Content.ReadAsStringAsync();

                var context = new TencentWeChatAuthenticatedContext(Context, JObject.Parse(userString), accessToken,
        ...
        private async Task<bool> InvokeReplyPathAsync()
        {
            if (Options.CallbackPath.HasValue && Options.CallbackPath == Request.Path)
            {
                ...
                var context = new TencentWeChatReturnEndpointContext(Context, ticket)
                {
        ...
        protected override Task ApplyResponseChallengeAsync()
        {
            ...
            if (challenge != null)
            {
                ...
                var context = new TencentWeChatApplyRedirectContext(Context, Options, authorizationEndpoint, properties);
